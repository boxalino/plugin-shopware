...
class HttpP13n {
     ...
     */
    protected function addRequestContext($requestLike) {
        if (
            $requestLike instanceof \com\boxalino\p13n\api\thrift\AutocompleteRequest ||
            $requestLike instanceof \com\boxalino\p13n\api\thrift\ChoiceRequest ||
            $requestLike instanceof \com\boxalino\p13n\api\thrift\BatchChoiceRequest
        ) {
            if ($requestLike->requestContext === null) {
                $requestLike->requestContext = $this->getRequestContext();
            } else {
                $requestContext = $this->getRequestContext();
                foreach ($requestContext->parameters as $parameterName => $parameterValues) {
                    if (isset($requestLike->requestContext->parameters[$parameterName])) {
                        continue;
                    }
                    $requestLike->requestContext->parameters[$parameterName] = $parameterValues;
                }
            }
        } else if ($requestLike instanceof \com\boxalino\p13n\api\thrift\AutocompleteRequestBundle) {
            foreach ($requestLike->requests as $request) {
